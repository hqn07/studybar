...
- **Schedule, redesigned** — a real weekly timetable instead of a day-by-day list. Weekday columns (Mon–Fri, with Sat/Sun appearing only if you have classes then) over a time ruler, with each class drawn as a block that spans its actual length — so a one-period lecture and a two-period lab read at a glance. Today's column is highlighted with a live now-line, blocks are course-colored, and you can tap a block to edit it or tap an empty slot to add a class there. Crucially, **a class can now meet on several days** — MWF is one class, not three: the editor has a day picker (M T W R F Sa Su). A ⋯ menu action **merges duplicate classes** (identical except for day) into one, and an opt-in **class-periods mode** labels the UF period grid (1–11, E1–E3), lets you enter a class by period, and shows each block's period span.

- **Notes, upgraded** — the editor gains checklists (tap a box to check it off), numbered lists, block quotes and dividers, with inline formatting preserved when you list-ify a line. Lists are now smart: **Tab / Shift-Tab** indents an item and **Return** starts the next one (or ends the list on an empty item). In the window, Notes is now a **two-pane workspace** — the note list and editor sit side-by-side, so you can browse and switch without leaving what you're writing (it falls back to the single-note view when the window is narrow). Notes can **link to each other**: type `[[` to autocomplete a note title, click a link to jump there (or create it), and a "Linked from" bar shows what points back. Every formatting button now shows its name on hover.
- **Smart typing (opt-in)** — with a local Ollama engine, Notes can suggest the next few words as you type, shown in grey inline; press Tab to accept. Turn it on in Settings ▸ Intelligence ▸ Smart typing. It runs entirely on your Mac and finishes your phrasing, never your homework.
